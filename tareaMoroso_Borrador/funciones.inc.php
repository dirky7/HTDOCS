...
function conexion_bd()
{
	try
	{
		...
		$dsn = "mysql:host=localhost;dbname=morosos";
...
function inserta_usuario($login, $password, $email, $con)
{
	try
	{
		$password_hash = crypt($password, 'BD');
		$sql = "INSERT INTO anunciantes values ('$login', '$password_hash', '1' ,'$email')";
		if ($con->exec($sql))
			return (true);
	}
	catch (PDOException $er)
...
function getBloqueados($con)
{
	try
	{
		$consulta = "SELECT * FROM anunciantes WHERE bbloqueado = 1";
